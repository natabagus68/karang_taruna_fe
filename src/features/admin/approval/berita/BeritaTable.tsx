  ...
  return (
    <>
      <ModalDecline
        ...
        onCancel={table.onCancelDecline}
        onChange={table.onChangeDecline}
        onOpen={table.openDecline}
        onSubmit={table.confirmDecline}
        open={table.decline}
      />
      <ModalConfirm
        open={table.openConfirm}
        setOpen={table.closeConfirm}
          ...
          <tbody>
            {!table.loading &&
              table.data?.data?.map((item) => {
                return (
                  <>
                    <tr key={item.id} className="border border-b-[#D0D3D9]">
                      <td className="py-5 pl-12">{item?.date ?? ""}</td>
                      <td className="py-5 pl-12">{item?.time ?? ""}</td>
                      <td className="py-5 pl-12">{item?.headline ?? ""}</td>
                      <td className="py-5 pl-12">{item?.redaktur ?? ""}</td>
                      <td className="py-5 pl-12 flex gap-2">
                        ...
                        <Button
                          onClick={() => table.handleOpenConfirm(item.id)}
